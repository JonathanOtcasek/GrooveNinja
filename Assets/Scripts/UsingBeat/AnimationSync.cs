...
public class AnimationSync : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Beat.Clock.clok.TimeMS >= 0d)
        {
            //Start playing the current animation from wherever the current beat loop is
            animator.Play(currentState, -1, GetNormalizedLoopPosition(Beat.Clock.clok.TimeMS));
    ...
    public float GetNormalizedLoopPosition(double timeMS)
    {
        ...
        float timeInBeats = (float)timeMS * 0.001f / Beat.Clock.clok.BeatLength();
